...
- `jea data migrate-activation-ledger [--subject NAME] [--dry-run] [--json] [--yes]`：把 pre-#233 / v1 单文件 Activation Ledger 拆成有界 hot open work、generation-scoped terminal shards 和 compact projection。不重建权威 evidence，不编造 identity；`sequence` 为 null 时写成 `0`。产品启动只读 projection，不解析终态历史。
...
- evidence journal 是可重建 sidecar，不是权威 evidence。默认 256 MiB 进入 `maintenance_due`、768 MiB 进入 `blocked`；可用 `JEA_EVIDENCE_JOURNAL_ROTATE_BYTES` / `JEA_EVIDENCE_JOURNAL_BLOCK_BYTES` 调整。heartbeat 只写有界状态，不在 live worker 下自动重写 journal。
...
- **依赖安装**：根目录 `.npmrc` 已设 `legacy-peer-deps=true`。OpenAI 7 / Claude Agent SDK 0.3 之后，zod peer 已对齐；剩余冲突是 `@jea/desktop` 的 `electron-vite@5.0.0`（peer `vite@^5 || ^6 || ^7`）与工作区 `vite@8` / `@vitejs/plugin-react@6`。上游 `electron-vite` 声明 vite 8 后即可删除该开关，并在干净环境验证 `npm install` 无 `ERESOLVE`。`npm ci` / `npm install` 会自动带上，不必再手写 `--legacy-peer-deps`。
