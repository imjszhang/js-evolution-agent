...
const FIXTURE_PAYLOADS: Record<string, Record<string, unknown>> = {
  ...
  'evolution.getReactorProgress': { subject: 'alpha' },
...
const TYPED_CALLS: Array<{ name: string; run(client: ReturnType<typeof createMemoryJeaClient>): Promise<unknown> }> = [
  ...
  { name: 'service.getReadiness', run: (client) => client.getServiceReadiness('alpha') },
  { name: 'service.requestCycle', run: (client) => client.requestCycle('alpha', 'fixture') },
  { name: 'service.processCycleOnce', run: (client) => client.processCycleOnce('alpha') },
  { name: 'service.setAutomation', run: (client) => client.setAutomation('alpha', 'automatic') },
