...
describe('Web SSE resume', () => {
  it('resumes from Last-Event-ID without duplicating applied events', async () => {
    ...
    const transcript = {
      ...
      duplicates: secondPass.events.filter((item) => applied.has(Number(item.seq))).map((item) => item.seq)
    }
    ...
    expect(transcript.replay_seq.every((seq) => seq > Number(first.cursor) || Number.isFinite(seq))).toBe(true)
    ...
    const stop = client.subscribe((event) => {
      ...
      seen.add(seq)
      clientEvents.push(seq)
    })
    await viWaitFor(() => clientEvents.includes(late.seq))
    stop()
