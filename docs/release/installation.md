...
- 产物：`JEA-0.2.1-macos-arm64.dmg` 或 `.zip`，外加 `SHA256SUMS`
- 不在 0.2.1 范围：Intel macOS、Windows、Linux 安装包、自动更新、npm 发布

## 安装步骤

1. 从 GitHub Release 选择 arm64 产物，并对照 `SHA256SUMS`。
2. 0.2.1 默认是 **ad-hoc 签名、未公证**。Gatekeeper 会拦截；请在 Finder 中右键 `JEA.app` → 打开。不要把应用放到随机下载目录长期运行。
...
开发者从源码运行 Desktop（`npm run desktop:dev`）不是 0.2.1 安装路径。
