...
function resolveReceiptAction(action, result) {
  if (action && typeof action === 'object' && !Array.isArray(action)) return action;
  if (result?.action && typeof result.action === 'object' && !Array.isArray(result.action)) {
    return result.action;
  }
  const type = action?.type ?? result?.action_type ?? 'unknown';
  return { type };
}

function verifyReceiptContract(action, result) {
  const resolvedAction = resolveReceiptAction(action, result);
  const receipt = {
    id: `receipt-${action?.id ?? resolvedAction?.id ?? 'unknown'}`,
    recorded_at: new Date().toISOString(),
    action_type: resolvedAction.type ?? 'unknown',
    action: resolvedAction,
    result: result && typeof result === 'object' && !Array.isArray(result) ? result : {},
  };
  handleContractValidation('action_receipt', validateActionReceipt(receipt));
}
  ...
  Object.keys(actionHandlers).map((type) => [
    type,
    {
      verify(action, result) {
        verifyReceiptContract(action, result);
