...
| 模块 | 目录 | Owner | 说明 |
| --- | --- | --- | --- |
| 共享内核 | `src/contracts`、`src/infra`、`src/domain` | agent-kernel | schema、原子写、subject registry/路径、runtime-paths |
...
| Daemon 编排 | `src/daemon` | agent-daemon | task queue、worker、step runner、cycle-state、evolve runs |
| Channel | `src/channel` | agent-channel | classifier / presence / speech / notify / control、飞书适配器 |
| 观测与门面 | `tools/evolution-viewer`、`src/cli`、`src/bridge` | agent-facade | 只读 viewer、CLI 命令薄壳、openclaw bridge |
...
| `src/infra/runtime-paths.mjs` | 所有模块 | subject runtime 路径与 subject lock 包装 |
