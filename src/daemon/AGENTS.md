...
Rule backlog 使用独立的安全预算：每批默认最多 64 个事件、4 MiB hydrated payload、4 分钟墙钟，连续瞬态失败最多 3 次；catch-up 默认 4 批或 10 分钟。对应 `JEA_RULE_MAX_EVENTS`、`JEA_RULE_MAX_PAYLOAD_BYTES`、`JEA_RULE_MAX_WALL_MS`、`JEA_RULE_MAX_CONSECUTIVE_FAILURES`、`JEA_RULE_CATCHUP_MAX_BATCHES`、`JEA_RULE_CATCHUP_MAX_WALL_MS`。确定性容量失败会有界拆分；不可拆分的单条 evidence 写入 `reactor/archive/rule-quarantine.jsonl` 后才推进 Rule cursor。`rule_catch_up_budget`、`rule_poison_batch_circuit_open`、`rule_journal_capacity_exceeded` 会稳定暴露到 daemon projection/readiness，且不暂停 Channel worker。
