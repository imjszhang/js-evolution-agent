...
import {
  activationLedgerDeltasPath,
  activationLedgerPath,
} from '../evolution/reactor/paths.mjs';
...
export function readActivationLedgerStore(dataRoot, { env = process.env } = {}) {
  const filePath = activationLedgerPath(dataRoot);
  ...
  if (raw.schema_version != null && raw.schema_version !== REACTOR_CONTROL_PLANE_CONTRACT_VERSION) {
    return {
      status: 'degraded',
      reason: 'activation_ledger_schema_mismatch',
      generation: Number.isInteger(raw.generation) ? raw.generation : null,
  ...
  const entries = Array.isArray(raw.entries)
    ? raw.entries.map(compactLedgerEntry).filter(Boolean)
    : [];
  if (Array.isArray(raw.entries) === false && raw.entries != null) {
    return {
      status: 'degraded',
      reason: 'activation_ledger_invalid',
      generation: Number.isInteger(raw.generation) ? raw.generation : null,
