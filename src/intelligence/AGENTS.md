...
- `jea data evidence-journal rebuild --subject NAME --dry-run --json`：停机迁移预览；确认 Cycle/Channel 均停止后加 `--yes`。重建从权威 sources 流式生成 v3 generation，按 `evidence_key` 去重，cursor 无法精确映射时统一保留 consumed markers 并从 offset 0 安全重放；旧 sidecar 保留时间戳备份。回滚用 `evidence-journal rollback --backup ID`。
