...
function buildVerificationPrompt({ execResult, mechanicalVerification }) {
  ...
  return [
    ...
    'Separate execution facts, receipt schema, and model interpretation. execution_status says whether the action actually ran; schema_status/acceptance_status say whether the final receipt shape satisfied the host contract; summaries and reasoning are agent_claims. A schema_invalid receipt can still contain usable execution evidence, but it must report the schema issue separately.',
    'No Boundary, No Fact: missing, blocked, no-match, or empty resource observations must cite boundary.execution_root, boundary.resource_scope, boundary.resource_kind, and boundary.path before they can support any resource conclusion.',
    'No Layer, No Execution Conclusion: action outcomes must be read through execution, schema, and semantic layers. Missing layer metadata means the statement remains a receipt claim, not a verified execution fact.',
    ...
    'Remembered entries such as [action_receipts:receipt-...] agent_claim: ... are intentionally contextual leads. They do not need typed_evidence_refs; typed_evidence_refs are for Seen evidence only. Do not report Remembered agent_claim lines as inline-only structure gaps unless they lack a reopenable source address.',
    '',
      ...
      semantic_verified: [
        {
          ...
          provider: 'llm_only | claude_code_sdk | cursor_sdk | reasonix_cli | acp:* | unknown',
          fallback_used: false,
