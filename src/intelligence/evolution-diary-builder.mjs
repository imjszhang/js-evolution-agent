...
export function buildEvolutionDiaryPrompt({
  ...
  if (language === 'en') {
    return [
      ...
      '- Explicitly record Phase 4 goal assessment and Phase 4.5 goal auto-calibration when present in Machine Context. Include status, confidence, and reason for Phase 4; include status, reason, next_goal_id, and written count for Phase 4.5. If calibration was skipped, state the skipped reason instead of omitting it.',
  ...
  return [
    ...
    '- 如果 Machine Context 中存在 phase4 或 phase4_5，必须显式记录 Phase 4 目标评估与 Phase 4.5 自动校准结果。Phase 4 至少写出 status、confidence、reason；Phase 4.5 至少写出 status、reason、next_goal_id、written。若校准被 skipped，也要写明 skipped reason，不要省略。',
