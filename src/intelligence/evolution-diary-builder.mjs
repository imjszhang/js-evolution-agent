...
export function buildEvolutionDiaryPrompt({
  ...
  if (language === 'en') {
    return [
      ...
      '- Timeline authority: statements about this cycle\'s exec/verify/belief/assess/calibrate status MUST use phase2–phase4_5 checkpoint fields. phase1.timeline marks Phase 1 narrative as written at cycle start (system-state claims there describe the previous cycle). Claims inside phase2 receipts that assess/calibrate/belief_update are still pending are execution-time snapshots only; when writing the diary those steps have already finished — use phase3_5/phase4/phase4_5 checkpoints, and never put "goals_assess/goals_calibrate still pending" into bullets or carryover. If Phase 1 narrative conflicts with later checkpoints, explicitly say the Phase 1 claim is stale; never copy it into "What did not move" or carryover bullets.',
  ...
  return [
    ...
    '- 时间线权威：凡涉及本轮 exec/verify/belief/assess/calibrate 状态的陈述，必须以 phase2–phase4_5 checkpoint 字段为准。phase1.timeline 标明 Phase 1 叙事写于轮初（其中系统状态描述截至上轮末）。phase2 receipt 里「本轮 assess/calibrate/belief_update 仍 pending」只是执行时刻快照；写日记时这些步骤已完成，必须以 phase3_5/phase4/phase4_5 checkpoint 为准，禁止把「goals_assess/goals_calibrate 仍 pending」写入 bullets 或 carryover。若 Phase 1 叙事与后续 checkpoint 冲突，须显式写「Phase 1 报告中的 X 描述已过时」，不得照抄进「没有推进的地方」或 carryover bullets。',
