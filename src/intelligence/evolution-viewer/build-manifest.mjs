 ...
 * @param {number} eventLimit
 */
function normalizeDiaryPath(path) {
  return String(path ?? '').replace(/\\/g, '/').toLowerCase();
}

function indexDiariesByIntelCycle(runtimeRoot, store) {
...
export function buildEvolutionViewerDist({
  ...
  for (const record of records) {
    ...
    writeFileSync(
      join(outDir, 'rounds', `${cycleId}.json`),
