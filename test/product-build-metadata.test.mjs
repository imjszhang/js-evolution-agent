...
describe('package smoke commit certification', () => {
  it('fail-closes when installers exist without an embedded commit SHA', () => {
    ...
    writeFileSync(join(dir, 'JEA-0.3.1-macos-arm64.dmg'), 'dmg');
    writeFileSync(join(dir, 'JEA-0.3.1-macos-arm64.zip'), 'zip');
    writeFileSync(join(dir, 'SHA256SUMS'), [
      `${createHash('sha256').update('dmg').digest('hex')}  JEA-0.3.1-macos-arm64.dmg`,
      `${createHash('sha256').update('zip').digest('hex')}  JEA-0.3.1-macos-arm64.zip`,
    ].join('\n'));
